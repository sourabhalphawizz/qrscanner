    ...
    <div class="col-12">
        ...
        >
            ...
            <button
                ...
                class="profile-tabBtn w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]"
            >
                <i class="lab lab-transactions"></i>
                <span class="capitalize text-sm">{{
                    $t("button.subscriptions")
                }}</span>
            </button>
        </div>
                        ...
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                ...
                                <span class="db-list-item-text w-full sm:w-1/2">
                                    <span
                                        :class="
                                            statusClass(administrator.status)
                                        "
                                        >{{
        ...
        <div id="branches" class="profile-tabDiv">
            <BranchList :props="$route.params.id" />
        </div>
        <div id="subscriptions" class="profile-tabDiv">
            <div class="db-card">
                <!-- <div class="db-card-header">
                    <h3 class="db-card-title">
                        {{ $t("button.add_subscription") }}
                    </h3>
                </div> -->
                <div class="db-card-body">
                        ...
                        <div class="form-row">
                            <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                                ...
                                <vue-select
                                    ...
                                    id="plan_id"
                                    v-model="subscription.props.form.plan_id"
                                    :options="plans"
                                    label-by="name"
                                    value-by="id"
                                    :closeOnSelect="true"
                            ...
                            <div class="form-col-12 sm:form-col-6">
                                <label
                                    for="start_date"
                                    class="db-field-title required"
                                    >{{ $t("label.start_date") }}</label
                                >
                                <Datepicker
                                    hideInputIcon
                                    autoApply
                                    v-model="subscription.props.form.start_date"
                                    :enableTimePicker="false"
                                    :is24="false"
                                    :monthChangeOnScroll="false"
                                    utc="false"
                                    :input-class-name="
                                        subscription.errors.start_date
                                            ? 'invalid'
                                            : ''
                                    "
                                >
                                ...
                                <small
                                    class="db-field-alert"
                                    v-if="subscription.errors.start_date"
                                    >{{
                                        subscription.errors.start_date[0]
                                    }}</small
                                >
                            </div>
                            ...
                            <div class="form-col-12">
                                <div class="flex flex-wrap gap-3">
                                    ...
                                    >
                                        <i class="lab lab-save"></i>
                                        <span>{{ $t("button.add_subscription") }}</span>
                                    </button>
                                </div>
                            </div>
                        </div>
            ...
            <hr />
            <AdministratorSubscriptionList :props="$route.params.id" />
        </div>
    </div>
</template>
...
import AdministratorSubscriptionList from "./subscription/AdministratorSubscriptionList";
...
export default {
    ...
    mounted() {
        ...
        this.$store
            .dispatch("administrator/show", this.$route.params.id)
            .then((res) => {
    ...
    methods: {
        ...
        orderLists: function (page = 1) {
            ...
            this.$store
                .dispatch("administrator/myOrders", {
        ...
        planList: function (page = 1) {
            this.loading.isActive = true;
            this.search.page = page;
            this.$store
                .dispatch("plan/lists", this.search)
                .then((res) => {
                    this.loading.isActive = false;
                })
                .catch((err) => {
        ...
        addSubscription: function () {
            try {
                ...
                this.$store
                    .dispatch("administratorSubscription/save", {
                        form: {
                            plan_id : this.subscription.props.form.plan_id,
                            start_date: this.subscription.props.form.start_date,
                        },
                        id: this.$route.params.id,
                    })
                    .then((res) => {
                        ...
                        alertService.successInfo(
                            tempId === null ? 0 : 1,
                            this.$t("message.subscription_update")
                        );
                        this.subscription.props.form = {
                            plan_id: null,
                            start_date: "",
                        };
                        this.subscription.errors = {};
                    })
                    .catch((err) => {
                        this.loading.isActive = false;
                        this.subscription.errors = err.response.data.errors;
                    });
            } catch (err) {
                this.loading.isActive = false;
                alertService.error(err);
            ...
            appService
                ...
                .then((res) => {
                    try {
                        this.loading.isActive = true;
                        this.$store
                            .dispatch("administratorSubscription/destroy", {
