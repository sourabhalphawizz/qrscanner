    ...
    <div id="modal" class="modal">
        <div class="modal-dialog">
            <div class="modal-header">
                <h3 class="modal-title">{{ $t("menu.plans") }}</h3>
                ...
                <form @submit.prevent="save">
                    <div class="form-row">
                        ...
                        <div class="form-col-12 sm:form-col-6">
                            ...
                            <input v-on:keypress="floatNumber($event)" v-model="props.form.price" v-bind:class="errors.price ? 'invalid' : ''" type="text"
                        ...
                        <div class="form-col-12 sm:form-col-6">
                            <label for="validity" class="db-field-title required">
                                {{ $t("label.validity") }}
                            </label>
                            <input v-on:keypress="onlyNumber($event)" v-model="props.form.validity" v-bind:class="errors.validity ? 'invalid' : ''"
                                type="text" id="validity" class="db-field-control" />
                            <small class="db-field-alert" v-if="errors.validity">{{ errors.validity[0] }}</small>
                        </div>
...
export default {
    ...
    computed: {
        addButton: function () {
            return { title: this.$t('button.add_plan') };
        },
    },
    methods: {
        ...
        reset: function () {
            ...
            this.$store.dispatch("plan/reset").then().catch();
            this.errors = {};
        ...
        save: function () {
            try {
                const tempId = this.$store.getters["plan/temp"].temp_id;
                this.loading.isActive = true;
                this.$store.dispatch("plan/save", this.props).then((res) => {
                    appService.modalHide();
